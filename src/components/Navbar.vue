  ...
  >
    ...
    >
      <router-link to="/" class="flex items-center">
        <span
          class="self-center font-sans text-lg sm:text-2xl font-semibold whitespace-nowrap dark:text-white"
          >Tailwinder</span
        >
      </router-link>
      <div class="flex items-center md:order-2">
        ...
        <button
          ...
          @click="themeToggle"
          type="button"
          class="my-2 text-gray-500 dark:text-gray-400 hover:bg-gray-400 dark:hover:bg-gray-700 focus:outline-none rounded-lg text-sm p-2.5"
        >
          <svg
            v-if="!dark"
            class="w-5 h-5"
      ...
      >
        ...
        >
          ...
          <li>
            <router-link
              :class="{ 'md:text-blue-500': $route.path === '/academy' }"
              class="block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-blue-500 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700"
              to="/academy"
            >
              Academy
...
export default {
  ...
  methods: {
    // Toggle between themes and apply condition
    themeToggle() {
      this.dark = !this.dark;
    ...
    // Update the user prefrences using appwrite web SDK
    updateUserTheme(theme) {
      try {
        appwrite.account.updatePrefs({
          bio: this.userPrefs.bio,
          country: this.userPrefs.country,
          github: this.userPrefs.github,
          twitter: this.userPrefs.twitter,
          theme: theme,
        });
      } catch (error) {
        console.log(error);
    ...
    // Get user prefrences using appwrite web SDK
    fetchUserTheme() {
      let promise = appwrite.account.getPrefs(store.userprofile.$id);
